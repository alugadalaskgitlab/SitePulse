...
- **Bituminous Mix Rate Calculator**: Interactive tool at `/mix-calculator` (vanilla HTML/JS), also accessible as a tab inside Admin Reports. Calculates ₹/MT, ₹/CUM, ₹/SQM rates for bituminous road layers (BC, DBM, SDBC, custom). Features: 2–4 configurable mix types with aggregate proportions, owned vs hired equipment modes, fuel & energy (HSD, LDO dryer, boiler), laying & compaction, transport, prime & tack coat costing. Job Estimator supports multiple mix types per job with full cost column breakdown (Plant ₹/MT, Plant Amt, Trans ₹/MT, Trans Amt, Lay ₹/MT, Lay Amt, Prime ₹, Tack ₹). Contractor Summary groups by contractor → mix type. Procurement Summary with per-material quantities. Scroll-wheel prevention on number inputs. Live recalculation on every input change with sticky rate summary panel. File: `client/public/mix-calculator.html`, route: `GET /mix-calculator` in `server/routes.ts`. **v4 State Format (storage key `hlc_mix_calc_v4`)**: Per-equipment `fuelConsump` (L/hr) fields — each equipment card (Hot Mix Plant, DG, JCB, Tipper, Paver, Roller, PTR) has its own fuel consumption input with defaults (hotmix:0, dg:12, jcb:8, tipper:4, paver:12, roller:6, ptr:5). HSD fuel cost is split between plant ops and laying equipment. Prime & Tack rows are always-on in job table (no checkboxes). Site/job hierarchy: jobs are nested inside named site cards (`sites[]` with `jobs[]`). Rate summary shows ₹/MT + ₹/CUM (blue) side-by-side for each mix. Procurement section includes LDO (Dryer+Boiler) and HSD (all equipment) rows. `migrateState()` handles v3→v4 auto-upgrade. `mixComparisonData.ts` supports both legacy flat `jobs[]` and new `sites[].jobs[]` state shapes. **Database-backed Saved Estimates**: 💾 Save button in calculator top bar POSTs/PUTs state JSON to `mix_estimates` table via `/api/mix-estimates` CRUD. 📂 Saved button shows modal list with Load and Delete actions. `currentEstimateId` tracked in-page + persisted via localStorage for cross-reload continuity. React list page at `/admin/mix-estimates` shows all saved estimates grouped by contractor with Load-in-Calculator, Delete, Price Impact, and New Site actions. "Saved Estimates" link appears on the Mix Calculator tab in Admin Reports. Contractor groups support inline rename (pencil icon) with case-insensitive bulk update via `PATCH /api/mix-estimates/rename-contractor`. Startup migration auto-fixes NULL contractor values from saved state JSON. **Price Impact Analysis** (`/admin/mix-impact`): React page for scenario-based cost impact analysis. Users select a saved estimate, create named scenarios (each opening the Mix Calculator in "Scenario Edit Mode"), and compare full calculator states side-by-side. `price_scenarios` DB table: `estimateId`, `name`, `revisedPrices` (legacy default `{}`), `state` (full JSON, nullable), `baseState` (frozen base snapshot JSON, nullable, write-once), `updatedAt` (nullable timestamp), `createdAt`. Each scenario stores an immutable `baseState` snapshot (the estimate state at scenario creation time) so comparisons always use a frozen base rather than the live estimate state. API: `GET/POST /api/price-scenarios`, `GET/PATCH/DELETE /api/price-scenarios/:id`. `diffCalcInputs(base, revised)` in `mixCalc.ts` generates a diff of all changed input keys. ScenarioComparison shows three tables: changed inputs, mix-type rates, and job-wise cost impact. Scenarios with full `state` use `calcMixRatesAndJobs(scState)` directly; legacy scenarios fall back to `revisedPrices` overrides. Timestamps shown as "Saved 15 Mar 26, 10:42 AM · Edited 17 Mar 26, 3:15 PM". **Scenario Edit Mode** in calculator: URL param `?scenarioId=<id>&estimateId=<eid>` triggers a blue banner with "Save Scenario & Return" button; state is PATCHed to `/api/price-scenarios/:id`; `saveState()` skips localStorage during scenario sessions. Print button available. **Scenario Comparison** (`/admin/scenario-comparison`): React page for side-by-side multi-scenario comparison within a single estimate. Users select scenarios via checkboxes and see three comparison tables: (1) Final Laid Rate by mix type with green/red highlighting for best/worst, (2) Site-wise cost comparison with deltas, (3) Summary with total cost, avg ₹/MT, and savings vs base. Accessible via "Compare Scenarios" button on the Price Impact page (shown when 2+ scenarios exist). Legacy scenarios without full `state` are disabled. Print-ready. **Contractor Comparative Rate Statement** (`/admin/mix-comparison`): React page comparing all contractors' mix rates and jobs in one view. Section 1: rate grid showing Final Laid ₹/MT per mix type per contractor with green (lowest) / red (highest) highlighting. Section 2: job ledger grouped by contractor with subtotals and grand total (columns: Contractor, Estimate/Job, MT, Plant ₹/MT, Trans ₹/MT, Lay ₹/MT, Total ₹/MT, Amount). Export to Excel (two sheets: Rate Comparison + Job Ledger). Print-ready. "Comparative Report" button in MixEstimates header navigates to this page.
    - **Mob/Demob Overhead** (Section ⑥b): Lump-sum Mobilisation and Demobilisation cost inputs, auto-amortized over grand total MT. Overhead ₹/MT flows into `allCostBeforeMargin` so it's included in margin calc and Final Laid Rate. Shows in rate summary (conditional), full breakdown, cost bar, and scenario diffs.
    - **Unified LDO Rate**: Boiler fuel rate input removed — boiler calculations now use the single LDO Rate from the Dryer section. Boiler card shows LDO rate as read-only derived field. `boilerFuelRate` removed from `PERSIST_INPUTS` and `INPUT_LABELS`. Old saved states with separate `boilerFuelRate` are handled gracefully.
    - **Cursor Fix**: Text inputs (mix name, site name, contractor) use inline `selectionStart`/`selectionEnd` save/restore to prevent cursor jumping to end on `toUpperCase()`. Global `input` event listener also preserves cursor for all non-number/date text inputs.
    - **Mix Type Display Filter**: Rate summary and full breakdown only show mix types assigned to at least one job (fallback: show all if no jobs exist). Cost bar uses full `mixRates` array since it always shows the active tab.
