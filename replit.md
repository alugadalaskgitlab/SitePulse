...
- **Concrete Rate Analysis Calculator** (planned — Task #98): BOQ-based rate analysis for civil structures (drains, box culverts, bridges, retaining walls). Full build spec:
    ...
    - **Petty Labour Contract mode (Task #109)**: Toggle in ⑤ Placement section with ₹/m³ or ₹/RM rate input. Scope checkboxes: `contractorFormwork` (bypasses ⑥ Formwork — cost forced to ₹0 with amber banner) and `contractorBBS` (informational only — "Contractor handles Bar Bending & Fixing"; steel material cost from BBS always applies). Rate auto-converted from ₹/RM→₹/m³ using QTO cross-section area (totalRCC ÷ totalLength). The contract rate replaces placement cost in computeCosts. pettyLabourRatePerM3 is propagated to all computeCosts callsites (main, revisedCosts, applyChangesToState) so Price Impact and Scenario analysis stay consistent.
    - **Restructured Cost Breakdown (Task #109)**: Rate Summary bar and right-column Rate Breakdown panel now use 4 grouped blocks — Raw Materials / Steel / Plant+Labour+Formwork / Overhead+Margin — each with sub-totals. Grand total displayed in both ₹/m³ and ₹/RM (when QTO cross-section available).
    - **Client's Offered Rate + ₹/RM profitability (Task #109)**: `contractRateMode: "per_m3"|"per_rm"` in CalcState. Right-column input relabelled "Client's Offered Rate" with ₹/m³ / ₹/RM toggle; "Client's Rate" column label in BOQ table and Contract Profitability table. Sticky section headers (`sticky top-14 z-10`) on all CardHeaders across all 4 tabs (20 headers total).
    ...
    - **Rate Breakdown Grade Selector (Task #119)**: Independent grade dropdown (M10–M40) in the Rate Breakdown card header; selecting a grade recalculates all breakdown bars and totals using that grade's mix design while all other costs (batching, placement, formwork, curing, overhead, margin) remain as-per-estimate. "(Main)" annotation shown next to the current Mix Design grade; "(PCC)" annotation for the QTO-configured PCC grade. PCC toggle button auto-activates when the breakdown grade matches `elementGrades.pcc`; toggleable manually otherwise. PCC mode zeroes out steel, formwork, placement, and labour, disables steel/formwork wastage, and lets overhead+margin recalculate on materials+batching+curing only; amber "PCC mode" banner shown. Progress bar max-bar computed from `bdCosts` values (not the main `costs`) for accurate bar widths.
