...
- Versioning for DPRs for audit trails with `isSuperseded` flag to filter out old versions.
...
- **Personnel Tracking Per Activity**: `personnel` table (name, role, phone, isActive) and `activity_personnel` junction table link personnel to each activity row. Personnel picker on each progress row in SiteEntry/SiteEdit forms with inline "Add Personnel" dialog (PIN-gated: Manager/Admin PIN required). Personnel names shown in DPR preview and report views. Personnel Master UI in Plant.tsx Masters tab with add/edit/toggle active functionality. `PERSONNEL_ROLES = ["Engineer", "Supervisor", "Assistant", "Foreman", "Other"]`.
- **Engineer Dropdown from Personnel Master**: The engineer field in SiteEntry/SiteEdit is a Select dropdown populated from the Personnel Master, showing "NAME - ROLE" format (e.g., "RAJU - ENGINEER"). Historical DPR engineer values are migrated on startup to match personnel master format where possible. Legacy values that don't match any personnel are shown as "NAME (LEGACY)" in the edit dropdown.
- **DPR Version Deduplication**: Uses `isSuperseded` boolean flag on `dprs` table. When a DPR is edited via `createVersionDpr`, the original is marked `isSuperseded=true`. `getDprs()` and `getDprsWithDetails()` filter by `isSuperseded=false`. Startup migration `migrateSupersededDprs()` retroactively marks old versions using both `dpr_versions` table entries and legacy baseSiteName+date grouping.
- **Uppercase Convention**: All text inputs across the entire app convert to uppercase on typing with `.toUpperCase()` + `className="uppercase"`. This includes all DPR fields, plant module fields, personnel names/phones, notes fields, etc.
