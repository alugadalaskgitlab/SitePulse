...
export default defineConfig({
  plugins: [react()],
  resolve: {
    // The mounted page is the production DieselRequirements component.  Only
    // its auth, origin, feature-flag, filter, and toast boundaries are
    // replaced, so the fixture has no session or database dependency.
    alias: [
