...
function StatusBadge({ status }: { status: string }) {
  if (status === "pending_stores")
    return <Badge className="bg-amber-50 text-amber-700 border border-amber-200 font-medium text-sm gap-1"><Clock className="h-3 w-3" />Pending Stores</Badge>;
  if (status === "stores_verified")
    return <Badge className="bg-blue-50 text-blue-700 border border-blue-200 font-medium text-sm gap-1"><Clock className="h-3 w-3" />Awaiting Approval</Badge>;
  if (status === "approved")
    return <Badge className="bg-green-50 text-green-700 border border-green-200 font-medium text-sm gap-1"><ShieldCheck className="h-3 w-3" />Approved</Badge>;
  if (status === "issued")
    return <Badge className="bg-emerald-100 text-emerald-800 border border-emerald-300 font-medium text-sm gap-1"><PackageCheck className="h-3 w-3" />Issued</Badge>;
  if (status === "partially_issued")
    return <Badge className="bg-orange-50 text-orange-700 border border-orange-300 font-medium text-sm gap-1"><PackageCheck className="h-3 w-3" />Partial Issue</Badge>;
  if (status === "rejected")
    return <Badge className="bg-red-50 text-red-700 border border-red-200 font-medium text-sm gap-1"><XCircle className="h-3 w-3" />Rejected</Badge>;
  return <Badge className="bg-gray-100 text-gray-600 border border-gray-200 font-medium text-sm gap-1"><Archive className="h-3 w-3" />Closed</Badge>;
}
...
function UrgencyDot({ items }: { items: InternalRequisitionWithItems["items"] }) {
  ...
  if (hasUrgent) return <span className="inline-flex items-center gap-1 text-sm text-red-600"><AlertTriangle className="h-3 w-3" />Urgent</span>;
  if (hasHigh) return <span className="text-sm text-orange-600">High</span>;
          ...
          <div key={header.irnId} className="p-4 space-y-3">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <span className="text-sm text-gray-400">{header.raisedFrom}</span>
                ...
                {hasPi && piId && (
                  <button
                    ...
                    className="inline-flex items-center gap-1 text-sm px-2 py-0.5 rounded border bg-indigo-50 border-indigo-200 text-indigo-700 hover:opacity-80"
                  >
            ...
            <div className="bg-gray-50 rounded-lg border overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-gray-100 border-b">
                  <tr>
                    <th className="text-left px-3 py-2 text-sm font-semibold text-gray-500 uppercase">Material</th>
                    <th className="text-left px-3 py-2 text-sm font-semibold text-gray-500 uppercase">Procure Qty</th>
                    <th className="text-left px-3 py-2 text-sm font-semibold text-gray-500 uppercase">Urgency</th>
                    <th className="text-left px-3 py-2 text-sm font-semibold text-gray-500 uppercase">Need By</th>
                    <th className="text-left px-3 py-2 text-sm font-semibold text-gray-500 uppercase">Status</th>
                  </tr>
                </thead>
                  ...
                  {items.map(item => (
                    <tr key={item.itemId}>
                      <td className="px-3 py-2">
                        ...
                        {item.storesNotes && <p className="text-sm text-blue-600 mt-0.5">Stores: {item.storesNotes}</p>}
                      </td>
                      <td className="px-3 py-2 font-semibold text-gray-800">
                        {item.procureQty ?? item.qty} <span className="font-normal text-gray-500">{item.uom}</span>
                      </td>
                      <td className="px-3 py-2">
                        <span className={`text-sm px-1.5 py-0.5 rounded-full border font-medium ${URGENCY_COLOR[item.urgency] ?? URGENCY_COLOR.normal}`}>
                      ...
                      <td className="px-3 py-2">
                        {item.itemStatus === "queued_procurement" ? (
                          <span className="inline-flex items-center gap-1 text-sm text-purple-700 bg-purple-50 border border-purple-200 px-1.5 py-0.5 rounded-full">
                            <ListTodo className="h-3 w-3" /> Queued
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 text-sm text-blue-700 bg-blue-50 border border-blue-200 px-1.5 py-0.5 rounded-full">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Raise and track material requests through stores</p>
            </div>
          </div>
          {canRaise && (
            <Link href="/irn/new">
          ...
          {statusFilter === "procurement_queue" ? (
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b">
                <tr>
                  <th className="text-left px-4 py-3 text-sm font-semibold text-gray-500 uppercase tracking-wide">IRN No.</th>
                  <th className="text-left px-4 py-3 text-sm font-semibold text-gray-500 uppercase tracking-wide">Raised By</th>
                  <th className="text-left px-4 py-3 text-sm font-semibold text-gray-500 uppercase tracking-wide">Section</th>
                  <th className="text-left px-4 py-3 text-sm font-semibold text-gray-500 uppercase tracking-wide">Date</th>
                  <th className="text-left px-4 py-3 text-sm font-semibold text-gray-500 uppercase tracking-wide">Items</th>
                  <th className="text-left px-4 py-3 text-sm font-semibold text-gray-500 uppercase tracking-wide">Urgency</th>
                  <th className="text-left px-4 py-3 text-sm font-semibold text-gray-500 uppercase tracking-wide">Status</th>
                  <th className="text-left px-4 py-3 text-sm font-semibold text-gray-500 uppercase tracking-wide">Action By</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredIrns.map((irn) => (
                  ...
                  >
                    <td className="px-4 py-3 font-mono text-sm font-semibold text-amber-700">{irn.irnNo}</td>
                    ...
                    <td className="px-4 py-3 text-gray-600">
                      <span className="text-sm text-gray-400 block">{irn.items.length} item{irn.items.length !== 1 ? "s" : ""}</span>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {(irn as any).linkedPiId && (
                          <Badge className="bg-indigo-50 text-indigo-700 border border-indigo-200 font-medium text-sm gap-1">
