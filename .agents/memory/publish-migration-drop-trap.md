...
- **Tooling trap:** in the installed Drizzle version, `push` with `tablesFilter` filters database introspection but can leave the full TypeScript schema in the diff, producing unrelated CREATE/DROP statements. Do not use filtered push for drift repair.
- **How to verify:** use Replit's development→production schema-diff preview after alignment. Completion means zero statements and no structural-data-loss warning; do not infer success from application startup.
