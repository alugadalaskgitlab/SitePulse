...
- **Plant Operations / Management Tab Layout**: Operator workflows (Plant Shift Log, Today's Shift Log, Bitumen Heating Sessions) live on the Plant **Operations** tab (no PIN required). Reports and stock tools (Today's Daily Plant Report, Stock Balances, Variance, Audit, Bitumen/LDO trackers, etc.) live on the **Management** tab (PIN-gated). Back buttons on Shift Log, Heating Sessions, and Daily Plant Report preserve the originating `tab`/`role` query params so users return to the correct tab.
