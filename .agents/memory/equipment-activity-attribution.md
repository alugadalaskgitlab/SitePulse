...
**How to apply:** Read normalized segments first and fall back to unchanged legacy rows when normalized segments are absent; do not bulk-backfill historical DPRs. Convert one equipment log only after a Work Assignment interaction marks that row dirty. Legacy-derived editor values must not trigger mount-time time/programme normalization. Persisted untouched rows omit assignment children from client payloads so replacement/version flows can carry them forward without assignment validation; explicit edits send normalized segments. Recalculate edited segment durations from same-day clock times and validate overlap between physical segments, duplicate BOQs within a segment, and the parent clock window. Meter working hours never cap clock segments. Programme links are silently reused only for one unique DPR-progress match; otherwise retain null.