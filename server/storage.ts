...
export interface IStorage {
  ...
  getBitumenTankBalances(partyId?: number | null): Promise<{ tank1: number; tank2: number; total: number }>;
...
export class DatabaseStorage implements IStorage {
  ...
  async createMaterialReceipt(receipt: InsertMaterialReceipt): Promise<MaterialReceipt> {
    ...
    return db.transaction(async (tx) => {
      ...
      // Update stock balance with row-level locking (prevents lost-update race conditions)
      const { newBalance } = await this._adjustStockBalance(tx, receipt.materialId, targetPartyId, stockQuantity, stockUom);
  ...
  async updateTruckDispatch(id: number, dispatch: Partial<InsertTruckDispatch>, adjustedBy?: string): Promise<TruckDispatch | undefined> {
    ...
    // Sync notes on non-system ledger dispatch entries when deliveryLocation changes.
    // createDispatch writes notes as "TemplateName — DeliveryLocation"; edits must mirror that.
    if (dispatch.deliveryLocation !== undefined) {
      const [updatedDisp] = await db
        .select({ mixTemplateId: truckDispatches.mixTemplateId, deliveryLocation: truckDispatches.deliveryLocation })
        .from(truckDispatches).where(eq(truckDispatches.id, id)).limit(1);
      if (updatedDisp?.mixTemplateId) {
        const [tpl] = await db.select({ name: mixTemplates.name })
          .from(mixTemplates).where(eq(mixTemplates.id, updatedDisp.mixTemplateId)).limit(1);
        if (tpl) {
          const newMixLabel = [tpl.name.trim(), updatedDisp.deliveryLocation?.trim() || null]
            .filter(Boolean).join(" \u2014 ");
          await db.update(stockLedger)
            .set({ notes: newMixLabel })
            .where(and(
              eq(stockLedger.transactionType, "dispatch"),
              eq(stockLedger.referenceId, id),
              sql`${stockLedger.notes} NOT LIKE '%[backfilled]%'`,
              sql`${stockLedger.notes} NOT LIKE '%[rebuild delta%'`,
            ));
        }
      }
    }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Shared stock-balance helper — used by every live API that adjusts stock.
  // Uses SELECT … FOR UPDATE so concurrent requests never produce lost updates.
  // delta > 0 = add, delta < 0 = deduct.
  // Returns the new balance so callers can embed it in the ledger entry.
  // ─────────────────────────────────────────────────────────────────────────
  private async _adjustStockBalance(
    tx: any,
    materialId: number,
    partyId: number | null,
    delta: number,
    ...
    const partyClause = partyId === null
      ...
      : sql`party_id = ${partyId}`;

    const locked = await tx.execute(sql`
      SELECT id, balance, uom
      FROM stock_balances
      WHERE material_id = ${materialId} AND ${partyClause}
      LIMIT 1
      FOR UPDATE
    `);
    ...
    if (row) {
      await tx.update(stockBalances)
        .set({ balance: newBalance, lastUpdated: new Date() })
        .where(eq(stockBalances.id, Number(row.id)));
      return { id: Number(row.id), newBalance };
    } else {
      const [inserted] = await tx.insert(stockBalances).values({
        materialId,
        partyId,
        balance: newBalance,
        uom: uom ?? 'Units',
      }).returning();
      return { id: inserted.id, newBalance };
    }
  }
  ...
  async createTruckDispatchWithStockDeduction(
    ...
    return db.transaction(async (tx) => {
      ...
      const deductFromSource = async (pId: number | null, matId: number, qty: number, uom: string, notes: string, tankNum?: number | null) => {
        // Lock the row for update to prevent concurrent dispatch races
        const lockResult = await tx.execute(
          pId === null
            ? sql`SELECT id, balance, uom FROM stock_balances WHERE party_id IS NULL AND material_id = ${matId} FOR UPDATE`
            : sql`SELECT id, balance, uom FROM stock_balances WHERE party_id = ${pId} AND material_id = ${matId} FOR UPDATE`
        );
        const existing = lockResult.rows[0] as { id: number; balance: string | number; uom: string } | undefined;
  ...
  async createMaterialIssue(issue: InsertMaterialIssue): Promise<MaterialIssue> {
    return db.transaction(async (tx) => {
      ...
      // in ldo_flow_readings so the tank tracker sees the fill.
  ...
  async updateMaterialIssue(id: number, issue: Partial<InsertMaterialIssue>): Promise<MaterialIssue | undefined> {
    return db.transaction(async (tx) => {
      ...
      // Reverse original stock impact only if there was a ledger entry for this issue.
        ...
        if (existingIssueLedger != null) {
          ...
          await this._adjustStockBalance(tx, original.materialId, originalStockPartyId, origStockQuantity, null);
        }
  ...
  async deleteMaterialIssue(id: number): Promise<boolean> {
    return db.transaction(async (tx) => {
      ...
      // Always restore stock balance and delete ledger entry on issue deletion.
        {
          // Reverse stock balance with row-level locking
          const stockPartyId = issue.isPlantCommon ? null : issue.partyId;
          await this._adjustStockBalance(tx, issue.materialId, stockPartyId, stockQuantity, null);
      ...
      // Remove linked ldo_flow_readings receipt row if present.
      await tx.delete(ldoFlowReadings).where(eq(ldoFlowReadings.sourceMaterialIssueId, id));
  ...
  async createMaterialReturn(ret: InsertMaterialReturn): Promise<MaterialReturn> {
    return db.transaction(async (tx) => {
      ...
      const { newBalance } = await this._adjustStockBalance(tx, ret.materialId, stockPartyId, stockQuantity, stockUom);
  ...
  async createMaterialOpeningStock(stock: InsertMaterialOpeningStock): Promise<MaterialOpeningStock> {
    return db.transaction(async (tx) => {
      ...
      // Update stock balance (row-level locking)
      const { newBalance } = await this._adjustStockBalance(tx, stock.materialId, stockPartyId, stockQuantity, stockUom);
  ...
  private async _syncLdoDipConsumptionToStock(
    ...
    const oldEntries = await tx.select().from(stockLedger).where(and(
      ...
      eq(stockLedger.date, date),
      sql`${stockLedger.notes} LIKE ${noteKey + '%'}`,
    ));
    ...
    // Re-read the pair after any in-transaction changes
    const [openingDip] = await tx.select().from(ldoDipReadings).where(and(
      eq(ldoDipReadings.date, date),
      eq(ldoDipReadings.tankNumber, tankNumber),
      eq(ldoDipReadings.readingType, "opening"),
      eq(ldoDipReadings.plantName, plantName),
    )).limit(1);
    const [closingDip] = await tx.select().from(ldoDipReadings).where(and(
      eq(ldoDipReadings.date, date),
      eq(ldoDipReadings.tankNumber, tankNumber),
      eq(ldoDipReadings.readingType, "closing"),
      eq(ldoDipReadings.plantName, plantName),
    )).limit(1);
    if (!openingDip || !closingDip) return;
  ...
  async createLdoDipReading(reading: InsertLdoDipReading): Promise<LdoDipReading> {
    return db.transaction(async (tx) => {
      const [existing] = await tx.select({ id: ldoDipReadings.id })
        .from(ldoDipReadings)
        .where(and(
      ...
      const uppercased = { ...reading, notes: reading.notes?.toUpperCase() };
      const [result] = await tx.insert(ldoDipReadings).values(uppercased).returning();

      const plantName = reading.plantName ?? "Main Plant";
      if (reading.readingType === "opening" || reading.readingType === "closing") {
        await this._syncLdoDipConsumptionToStock(tx, String(reading.date), reading.tankNumber, plantName);
      }
      return result;
    });
  }
  ...
  async updateLdoDipReading(id: number, updates: Partial<InsertLdoDipReading>): Promise<LdoDipReading | undefined> {
    return db.transaction(async (tx) => {
      const [before] = await tx.select().from(ldoDipReadings).where(eq(ldoDipReadings.id, id)).limit(1);
      ...
      const [result] = await tx.update(ldoDipReadings)
        .set(cleanUpdates)
        .where(eq(ldoDipReadings.id, id))
        .returning();

      // Sync old pair (if the row was opening/closing)
      if (before.readingType === "opening" || before.readingType === "closing") {
        await this._syncLdoDipConsumptionToStock(tx, before.date, before.tankNumber, before.plantName ?? "Main Plant");
      }
      // Sync new pair location if date/tank/plant changed
      const newDate = updates.date !== undefined ? String(updates.date) : before.date;
      const newTank = updates.tankNumber ?? before.tankNumber;
      const newPlant = (updates as any).plantName ?? before.plantName ?? "Main Plant";
      const newReadingType = updates.readingType ?? before.readingType;
      const newKey = `${newDate}:${newTank}:${newPlant}`;
      const oldKey = `${before.date}:${before.tankNumber}:${before.plantName ?? "Main Plant"}`;
      if (newKey !== oldKey && (newReadingType === "opening" || newReadingType === "closing")) {
        await this._syncLdoDipConsumptionToStock(tx, newDate, newTank, newPlant);
      }
      return result;
    });
  }
  ...
  async deleteLdoDipReading(id: number): Promise<boolean> {
    return db.transaction(async (tx) => {
      ...
      await tx.delete(ldoDipReadings).where(eq(ldoDipReadings.id, id));
      if (before.readingType === "opening" || before.readingType === "closing") {
        await this._syncLdoDipConsumptionToStock(tx, before.date, before.tankNumber, before.plantName ?? "Main Plant");
      }
      return true;
  ...
  async recordBulkMaterialReceipt(
    ...
    // Security: verify all itemIds belong to this indent
    const validItemIds = new Set(existing.items.map(i => i.id));
    for (const item of items) {
      if (!validItemIds.has(item.itemId)) {
    ...
    for (const item of items) {
      ...
      // Update running totals + link receipt on the indent item
      ...
      await db.update(purchaseIndentItems)
        .set({ totalAcceptedQty: newAccepted, purchaseStatus, linkedReceiptId: receipt.id, vendor: item.vendor?.toUpperCase() ?? null })
        .where(eq(purchaseIndentItems.id, item.itemId));
    }
    ...
    try {
      ...
      await db.transaction(async (tx) => {
        ...
        const badIssueLedger = await tx.select({ id: stockLedger.id })
          .from(stockLedger)
          .innerJoin(materialIssues, eq(stockLedger.referenceId, materialIssues.id))
          .where(and(
            eq(stockLedger.transactionType, "issue"),
            inArray(stockLedger.materialId, ldoMatIds),
            isNotNull(materialIssues.ldoTankNumber)
          ));

        if (badIssueLedger.length > 0) {
          await tx.delete(stockLedger).where(inArray(stockLedger.id, badIssueLedger.map(r => r.id)));
          result.receiptLedgerRemoved += badIssueLedger.length;
        ...
        type DipKey = string;
        const dipPairs = new Map<DipKey, { opening: any; closing: any; date: string; tankNumber: number; plantName: string }>();
        for (const row of allDipReadings) {
          const plant = row.plantName ?? "Main Plant";
          const key: DipKey = `${row.date}:${row.tankNumber}:${plant}`;
          if (!dipPairs.has(key)) dipPairs.set(key, { opening: null, closing: null, date: row.date, tankNumber: row.tankNumber, plantName: plant });
          const pair = dipPairs.get(key)!;
          if (row.readingType === "opening") pair.opening = row;
  ...
  async fixDoubleDeductedDispatchOwnerRows(): Promise<{ rowsFixed: number; materialsRecomputed: number }> {
    // Detect and correct double-deduction: an owner dispatch row has quantity_out > 0
    // while an HLC "Borrowed from HLC" row also has quantity_out > 0 for the same
    // dispatch + material — meaning HLC ALREADY covered the full deduction, so the
    // owner row should be zero.
    //
    // Two cases are detected (either is sufficient to confirm the row is erroneous):
    //   Case A — owner has NO material receipts for that material at all.
    //            The material scope belongs entirely to HLC; the owner row is always wrong.
    //   Case B — owner HAS receipts but shortage_warning.available === 0 at dispatch time.
    //            Their stock was fully exhausted; HLC covered the whole qty, yet the
    //            owner row still carries a positive qty_out.
    //
    // Fix: zero out quantity_out on confirmed owner rows, recompute balance_after for
    // each affected material, then reconcile stock_balances.
    //
    // Robustness:
    //   • HLC parties resolved dynamically (ILIKE '%HLC%' OR '%HIGH LANE%').
    //   • shortage_warning parsed per-row in TypeScript; malformed rows are skipped.
    //   • Idempotent: qty_out > 0 guard prevents re-running on already-fixed rows.
    const result = { rowsFixed: 0, materialsRecomputed: 0 };
    try {
      // 0. Resolve all HLC-family party IDs dynamically.
      ...
      const phaseAResult = await db.execute(sql`
        ...
        SELECT
          ...
          (SELECT hlc2.party_id FROM stock_ledger hlc2
           ...
           LIMIT 1) AS borrow_party_id
        FROM stock_ledger sl
        LEFT JOIN truck_dispatches td ON td.id = sl.reference_id
        WHERE sl.transaction_type  = 'dispatch'
          AND sl.reference_id      IS NOT NULL
      ...
      for (const row of phaseARows) {
        ...
        // Verify the borrow row belongs to an HLC-family party (when IDs are known).
        if (hlcPartyIds.length > 0) {
          const borrowPartyId = row.borrow_party_id != null ? parseInt(String(row.borrow_party_id), 10) : NaN;
  ...
  private async _syncShiftLogReadings(tx: typeof db, log: PlantShiftLog): Promise<string[]> {
    ...
    if (ldoDipRows.length) {
      await tx.insert(ldoDipReadings).values(ldoDipRows).onConflictDoNothing();
      // Sync store-stock deduction from dip pairs for each affected tank
      const affectedTanks = [...new Set(ldoDipRows.map((r: any) => r.tankNumber as number))];
      for (const tankNum of affectedTanks) {
        await this._syncLdoDipConsumptionToStock(tx, log.date, tankNum, log.plantName);
      }
    }
  ...
  async getBitumenTankBalances(partyId?: number | null): Promise<{ tank1: number; tank2: number; total: number }> {
      const [bitumenMat] = await db.select({ id: plantMaterials.id })
        .from(plantMaterials)
        .where(sql`UPPER(${plantMaterials.name}) LIKE '%BITUMEN%'`)
        .limit(1);
      if (!bitumenMat) return { tank1: 0, tank2: 0, total: 0 };

      // Per-tank balance — only rows with a tank number assigned (guidance for tank selector)
      const perTankConditions: ReturnType<typeof and>[] = [
        eq(stockLedger.materialId, bitumenMat.id),
        sql`${stockLedger.tankNumber} IS NOT NULL`,
      ];
      if (partyId != null) {
        perTankConditions.push(eq(stockLedger.partyId, partyId));
      }
      ...
      // True total: all rows regardless of tankNumber (includes older entries without tank assigned).
      // This is the correct "is there enough stock?" figure for the dispatch warning.
      const totalConditions: ReturnType<typeof and>[] = [
        eq(stockLedger.materialId, bitumenMat.id),
      ];
      if (partyId != null) {
        totalConditions.push(eq(stockLedger.partyId, partyId));
      }
      ...
      const total = totalRow ? Number(totalRow.quantityIn) - Number(totalRow.quantityOut) : 0;

      return { tank1, tank2, total };
    }
      ...
      return db.transaction(async (tx) => {
        const candidates = await tx.execute(sql`
          SELECT mi.id, mi.date, mi.material_id, mi.quantity, mi.uom,
                 ...
                 pm.conversion_factor, pm.conversion_from_uom, pm.conversion_to_uom
          FROM material_issues mi
          JOIN plant_materials pm ON pm.id = mi.material_id
          WHERE mi.ldo_tank_number IS NOT NULL
            AND (LOWER(pm.name) LIKE '%diesel%' OR LOWER(pm.name) LIKE '%ldo%')
            AND NOT EXISTS (
              SELECT 1 FROM stock_ledger sl
              WHERE sl.transaction_type = 'issue'
                AND sl.reference_id = mi.id
            )
        ...
        for (const row of candidates.rows as any[]) {
          try {
            ...
            if (row.conversion_factor && row.conversion_from_uom && row.conversion_to_uom) {
              if (String(row.uom).toUpperCase() === String(row.conversion_from_uom).toUpperCase()) {
                stockQty = Number(row.quantity) * Number(row.conversion_factor);
                stockUom = row.conversion_to_uom;
              }
            }
  ...
  // Deduct (or restore when reverse=true) plant stock for an RMC batch based on the
  // mix design's componentProportions (kg/m³ → Ton via totalVolumeM3 / 1000).
  // Materials are matched by fuzzy name against plantMaterials; unmatched keys are silently skipped.
  private async _deductRmcMaterials(
    tx: any,
    ...
    for (const [key, patterns] of keyToPatterns) {
      ...
      if (!reverse) {
        await tx.insert(stockLedger).values({
          ...
          balanceAfter: newBalance,
          uom: 'Ton',
          notes: `RMC ${design.grade} — ${volM3.toFixed(3)} m³`,
        });
      }
    }
  }
  ...
  async updateRmcBatchRecord(id: number, r: Partial<InsertRmcBatchRecord>): Promise<RmcBatchRecord | undefined> {
    return db.transaction(async (tx) => {
      ...
      if (!original) return undefined;
      // Reverse original stock consumption then delete old ledger entries
      await this._deductRmcMaterials(tx, original, true);
      await tx.delete(stockLedger).where(
        and(eq(stockLedger.transactionType, 'rmc_batch'), eq(stockLedger.referenceId, id))
      );
  ...
  async deleteRmcBatchRecord(id: number): Promise<boolean> {
    return db.transaction(async (tx) => {
      ...
      // Restore stock balances
      await this._deductRmcMaterials(tx, original, true);
      // Delete ledger entries and the batch record
      await tx.delete(stockLedger).where(
        and(eq(stockLedger.transactionType, 'rmc_batch'), eq(stockLedger.referenceId, id))
      );
      await tx.delete(rmcBatchRecords).where(eq(rmcBatchRecords.id, id));
      return true;
    });
  }
  ...
  async getWorkProgramBars(boqProjectId: number): Promise<WorkProgramBarWithItem[]> {
    const rows = await db
      .select({
        ...
        stage: workProgramBars.stage,
        sequenceOrder: workProgramBars.sequenceOrder,
