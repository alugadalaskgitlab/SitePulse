...
- **Procurement & Finance Module**:
    - **Purchase Indents**: Multi-item request system with partial approval, adjustable quantities, and status-driven workflow (pending, approved, completed, rejected). Includes item cancellation, force-close, and detailed history.
    ...
    - **Vendor Bills**: Date-wise billing system with a 4-step workflow (Draft → Verified → Approved → Paid). Automatically pulls line items from various vendor data sources, supports vendor aliases, grouped line items by category, distance-based transport billing, and adjustments. Enhanced with PDF export and print layouts. Features **Vendor Rate Cards** (`vendor_rate_cards` table) for persistent per-vendor per-item rates that auto-apply during bill creation and are saved back when SET RATES is used. Includes **duplicate billing detection** — items already billed in other vendor bills are flagged with a red BILLED badge showing the bill reference and status. Rate Card management UI at `/plant/rate-cards`. Rate card keys use machine-type grouping: equipment `EQ_{MACHINE_NAME}_{UNIT}` (e.g. `EQ_TIPPER_HRS`), materials `MAT_{NAME}_{UNIT}` (e.g. `MAT_20MM_AGGREGATE_CFT`). Unit column is an editable dropdown. Dual-key lookup: new format first, fallback to legacy `{equipmentId}_{ENTRY_TYPE}` / `MAT_{NAME}` keys.
