...
- An unchanged saved draft row is still a new claim at final submission; only submitted-version edits may exempt unchanged claims from prior-overlap warnings.
- **Why:** exempting persisted draft rows hid the inline reason control while the server correctly rejected their overlapping final submission. Persisted does not mean previously submitted.
- **How to apply:** base any unchanged-claim exemption on the saved DPR status, not merely the existence of persisted progress IDs; delayed prior-query results must still update draft warnings.
