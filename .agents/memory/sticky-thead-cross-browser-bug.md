...
- Do NOT make a secondary row (e.g. a category/group band inside the body) sticky underneath an already-sticky header. Two stacked sticky elements at different DOM levels (per-`<th>` header vs. a `<tr>` in `<tbody>`) don't hand off cleanly — the body's sticky band can render on top of / slice through whatever data row happens to be scrolling past at that exact pixel position, even with correct `top`/z-index math. Keep group/category divider rows in normal (non-sticky) flow; only the column header row itself should be sticky.
