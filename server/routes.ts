...
export async function registerRoutes(
  ...
  app.post("/api/boq/projects/:id/programme/clean-structure-bars", async (req, res) => {
    try {
      ...
      // Delete bars on structure items that are NOT a proper imported
      // structure-location bar AND are not a bar the planner truly placed
      // manually.  "Truly manual" means source="manual" with a custom label
      // that does NOT match the auto-label pattern above.
      const toDelete = (bars as any[]).filter((b) => {
        if (!structureItemIds.has(b.boqItemId)) return false;
        if (b.planningMode === "structure_location") return false; // keep imported bars
        // Auto-sequence / auto-generate bars
        if (b.source === "auto-sequence" || b.source === "auto_generate") return true;
        ...
        if (b.source === "manual" && AUTO_LABEL_RE.test(b.reachLabel ?? "")) return true;
        return false;
      });
