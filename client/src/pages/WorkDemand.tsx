...
function SuggestionBadge({
  ...
  if (ps === "mapping_required" || ps === "uom_resolution_required" || ps === "multiple_matches") {
    // Build context label for the resolve button
    const contextNote: string | null =
      rr === "inactive_material" && rd?.inactiveMaterialName
        ? `"${rd.inactiveMaterialName}" is inactive — activate or remap`
        : rr === "uom_incompatible" && rd?.materialName
  ...
  // §4D: future_not_due — no horizon demand, future demand exists; early arrangement is optional
    ...
    <div className="flex flex-col gap-1">
      ...
      <div className="flex flex-wrap gap-1">
        <button onClick={() => openFor("pi")} disabled={isPending}
          className="inline-flex items-center gap-1 text-[11px] text-slate-600 bg-slate-50 border border-slate-200 rounded px-1.5 py-0.5 hover:bg-slate-100 transition-colors disabled:opacity-60">
          <ShoppingCart className="w-3 h-3" /> Early PI (optional)
        </button>
        <button onClick={() => openFor("irn")} disabled={isPending}
          className="inline-flex items-center gap-1 text-[11px] text-slate-600 bg-slate-50 border border-slate-200 rounded px-1.5 py-0.5 hover:bg-slate-100 transition-colors disabled:opacity-60">
          <ShoppingCart className="w-3 h-3" /> Early IRN (optional)
        </button>
      </div>
    </div>
  );
        ...
        {ps === "partially_covered" && (
          <>
            <button onClick={() => openFor("pi")} disabled={isPending}
              ...
              <ShoppingCart className="w-3 h-3" /> Raise PI (Balance)
            </button>
            <button onClick={() => openFor("irn")} disabled={isPending}
          ...
          <>
            {row.stockElsewhere > 0 ? (
              ...
              <>
                ...
                <button onClick={() => openFor("irn")} disabled={isPending}
                  className="inline-flex items-center gap-1 text-[12px] font-semibold text-orange-700 bg-orange-50 border border-orange-200 rounded px-1.5 py-0.5 hover:bg-orange-100 transition-colors disabled:opacity-60">
                  <ShoppingCart className="w-3 h-3" /> Issue from HLC Stock
                </button>
              </>
            ) : (
              <button onClick={() => openFor("pi")} disabled={isPending}
        ...
        <table className="text-sm border-collapse" style={{ minWidth: 960 }}>
          <thead>
            <tr style={{ background: "#0F5F64" }}>
              ...
              <th className="px-3 py-2 font-semibold text-white text-left min-w-[150px] sticky top-0 z-20" style={{ background: "#0F5F64" }}>Status / Action</th>
            </tr>
          </thead>
          <tbody>
            {data.rows.map(row => {
