...
- **Plant Production Module**: Manages material receipts, mix templates, truck dispatches with owner-first stock routing and borrowing logic, and detailed equipment usage including fuel stock tracking. Includes a stock ledger reassignment tool, an inter-party stock transfer tool (for returning borrowed material between parties), and a **Dispatch Ledger Rebuild tool** (`/plant/ledger-rebuild`) that rewrites aggregate component ledger entries for a mix template from a chosen date+time cutoff using current component proportions.
- **Work Programme Planning Engine**: Full Gantt-style work programme with BOQ-item stretches, chainage support, monthly distribution, and overlap detection. BOM & Demand page (`/work-program/:id/demand`) provides 5 tabs: Materials by month, Equipment hours, Labour days, By Item (item-wise breakdown), and Procurement intelligence (shortage check against current plant stock with IRN/PI action links). Shortage API (`GET /api/boq/projects/:id/shortage-check`) matches BOM demand against plant stock by name and returns shortfall + suggested action per material.
