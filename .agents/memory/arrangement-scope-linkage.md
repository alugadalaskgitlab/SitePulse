...
**Reach revision reference lifecycle (verified live, Batch 03):** revising a confirmed reach always creates a NEW segment id (old row kept superseded, `revisionOf` link). While the revision is draft, arrangements keep pointing at the old id and the register renders "name (superseded)" — never blank. On confirm, `repointArrangementScopeLinks` rewrites `scopeSegmentIds` old→new. The pencil-edit blank-form bug was fixed by the scopeForm.ts extraction (hydration is unit- and DOM-tested in scopeConfirmedReachEditBatch03.test.tsx); a "still broken" report usually means a stale browser bundle, not a regression.
