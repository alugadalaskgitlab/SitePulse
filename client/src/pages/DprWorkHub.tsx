...
export default function DprWorkHub() {
  ...
  if (isError || !dpr) return <div className="rounded-2xl border border-destructive/30 bg-destructive/10 p-6"><p className="font-semibold">Today’s draft could not be opened.</p><Link href="/" className="mt-4 inline-flex items-center gap-2 text-sm underline">Return to Field Home</Link></div>;

  return (
    <main className="mx-auto max-w-2xl pb-10">
      <Link href="/" className="mb-5 inline-flex items-center gap-1 text-sm font-semibold text-muted-foreground"><ChevronLeft className="h-4 w-4" /> Field Home</Link>
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        {cards.map((card) => {
          ...
          return <Link key={card.section} href={dprWorkSectionHref(draftId, card.section)} className="group rounded-2xl border border-border bg-card p-4 shadow-sm transition-transform hover:-translate-y-0.5 hover:border-slate-400">
