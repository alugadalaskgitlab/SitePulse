...
export default function SiteEntry() {
  ...
  // Breakdown evidence is part of the DPR write, unlike general photos which
  // can be attached after a DPR exists.  Upload first and retain File state on
  // failure so Save never silently drops selected evidence.
  const prepareBreakdownAttachments = async (rows: EquipmentEntry[]) => Promise.all(rows.map(async (row) => ({
    ...
    breakdowns: await Promise.all((row.breakdowns ?? []).map(async (breakdown) => {
      ...
      // Store the object identity before the request. A network-ambiguous
      // retry then reuses this upload instead of creating duplicate objects.
  ...
  const handleRestoreDraft = useCallback((data: SiteEntryFormData) => {
    setHeader((current) => ({ ...current, ...data.header }));
  ...
  // Chainage is authoritative for road length. The shared helper falls back to
  // a stored length only for legacy/non-chainage rows.
  ...
  const applyYesterdayStructure = () => {
    ...
    const blankEq = { machine: "", vehicleNo: "", operator: "", task: "", entryType: "time_meter", startTime: currentLocalTime(), endTime: "", openingReading: null, closingReading: null, diesel: null, equipmentId: null, dieselSource: "", fuelStation: "", billNumber: "", amountPaid: null, numberOfTrips: null, tripDistance: null, totalKm: null, waterQuantity: null, boqItemId: null, structureId: null, plantUsageId: null, breakdowns: [] as StagedBreakdown[] };
          ...
          progress.map((entry, idx) => (
            <div key={idx} className="p-4 border rounded-lg bg-muted/30 space-y-3 transition-all duration-500" data-dpr-row-key={dprRowKey("activities", idx)} data-testid={`progress-row-${idx}`}>
              ...
              {entry.noSiteWork ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3" data-testid={`activity-identity-block-${idx}`}>
